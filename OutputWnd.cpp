...
COutputWnd::COutputWnd():RConnectionNode()
{
}
...
int COutputWnd::HandleMessage(RConnectionMessageType type, void* pData)
{
	switch (type)
	{
	case RConnectionMessageType::OPEN_NEW_IMAGE:
		m_wndOutputBuild.AddString(_T("打开图片 ")+*((CString*)pData));
		break;
	case RConnectionMessageType::OUTPUT_DATA:
		m_wndOutputBuild.AddString(*((CString*)pData));
		break;
	default:
		break;
	}
...
int COutputWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	ASSERT(bNameValid);
	m_wndTabs.AddTab(&m_wndOutputBuild, _T("输出"), (UINT)0);
...
{
	m_wndOutputBuild.AddString(_T("启动完成！"));
}
